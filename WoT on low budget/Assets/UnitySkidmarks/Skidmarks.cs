...
public class Skidmarks : MonoBehaviour {
	...
	const int MAX_MARKS = 4096; // Max number of marks total for everyone together
	const float MARK_WIDTH = 0.24f; // Width of the skidmarks. Should match the width of the wheels
	...
	const float MAX_OPACITY = 0.8f; // Max skidmark opacity
	...
	public int AddSkidMark(Vector3 pos, Vector3 normal, float opacity, int lastIndex, float Velocity) {
		if (opacity > 1) opacity = 1.0f;
		else if (opacity < 0) return -1;

		black.a = (byte)(opacity * 255);
		return AddSkidMark(pos, normal, black, lastIndex, Velocity);
	}

	// Function called by the wheel that's skidding. Sets the colour and intensity of the skidmark section.
	public int AddSkidMark(Vector3 pos, Vector3 normal, Color32 colour, int lastIndex, float Velocity) {
		...
		//bool SkidTooClose=false;
		if (lastIndex != -1) {
			lastSection = skidmarks[lastIndex];
			distAndDirection = newPos - lastSection.Pos;
			if (distAndDirection.sqrMagnitude < MIN_SQR_DISTANCE && Velocity > MIN_DISTANCE) {
				return lastIndex;
			}
		...
		UpdateSkidmarksMesh();
		//if (SkidTooClose) return lastIndex;
